...
- **Server** (`/packages/server`): Two entry points — **host** (`src/index.ts`, Bun.serve) and **enclave** (`src/enclave.ts`, Bun.serve). Zero `@aztec/*` runtime dependencies — calls `bb prove` CLI directly. Only 2 runtime deps: `@logtape/logtape` (structured logging) and `openpgp` (PGP encryption). Supports multi-version bb binaries via `BB_VERSIONS_DIR` cache and `x-aztec-version` request header. In `TEE_MODE=nitro`, host proxies to enclave; in `TEE_MODE=standard`, host handles everything (local dev). Enclave manages keys, attestation (with bb SHA256 hashes in `user_data`), decryption, and proving. Host manages bb downloads from GitHub releases and uploads to enclave at runtime. `/health` returns `api_version`, `available_versions`, `bb_hashes`, and runtime diagnostics. `/prove` returns `x-prove-duration-ms` and `x-decrypt-duration-ms` timing headers.
...
- **CI**: GitHub Actions (per-package workflows with gate jobs: `sdk.yml`, `app.yml`, `server.yml`, `accelerator.yml`; shell & workflow lint: `actionlint.yml`; auto-update: `aztec-nightlies.yml`, `aztec-stable.yml`; infra: `infra.yml`; deploy: `deploy-mainnet.yml`, `deploy-testnet.yml`, `deploy-nightlies.yml`; release: `release-accelerator.yml`; reusable: `_build-base.yml`, `_deploy-unified.yml`, `_publish-sdk.yml`, `_aztec-update.yml`, `_e2e-sdk.yml`, `_e2e-app.yml`)
...
- **Infrastructure** (`/infra/tofu`): OpenTofu (IaC) managing all AWS resources — EC2, EIPs, SG, IAM, ECR, S3, ACM, CloudFront. Single state file for ci/prod. Remote state in S3. Single c7i.12xlarge instance runs Nitro enclave (localhost:4000 via socat, not externally accessible) and host container (port 80, `--network host`) for all environments. All external traffic (both `/prover/*` and `/tee/*`) routes through the host on port 80 — the host proxies to the enclave internally. bb binaries are downloaded by the host at deploy time and uploaded to the enclave via `POST /upload-bb` — not baked into Docker images. 4 CloudFront distributions (mainnet, testnet, nightlies, prod/legacy) all route to the same instance; multi-version bb handles version routing via `x-aztec-version` header. CI instance (m5.xlarge, stopped by default) is separate.
- **Multi-environment strategy**: 3 environments served from 2 branches. `main` (v4.1.x stable) deploys to mainnet + testnet. `nightlies` (v5.x nightly) deploys to nightlies. All share one EC2 instance.
...
- **Accelerator HTTPS (Safari)**: Optional HTTPS on port 59834 for Safari compatibility (Safari blocks mixed content from HTTPS pages to `http://127.0.0.1`). Opt-in via tray menu "Safari Support" toggle (macOS only). Auto-provisions local CA + leaf cert, installs in macOS Keychain. SDK probes HTTP (59833) + HTTPS (59834) in parallel via `Promise.any`. Config: `~/.tee-rex-accelerator/config.json`, certs: `~/.tee-rex-accelerator/certs/`.
