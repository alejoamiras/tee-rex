...
- **CI**: GitHub Actions (per-package workflows with gate jobs: `sdk.yml`, `app.yml`, `server.yml`, `accelerator.yml`; shell & workflow lint: `actionlint.yml`; auto-update: `aztec-nightlies.yml`, `aztec-devnet.yml`; infra: `infra.yml`; deploy: `deploy-prod.yml`, `deploy-devnet.yml`; release: `release-accelerator.yml`; reusable: `_build-base.yml`, `_deploy-unified.yml`, `_publish-sdk.yml`, `_aztec-update.yml`, `_e2e-sdk.yml`, `_e2e-app.yml`)
