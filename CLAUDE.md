...
- **Playground** (`/packages/playground`): Vite + vanilla TS frontend — local/UEE/TEE mode toggle, timing, token flow, external wallet connection via `@aztec/wallet-sdk`
- **Build system**: Bun workspaces (`packages/sdk`, `packages/server`, `packages/playground`)
...
- **CI**: GitHub Actions (per-package workflows with gate jobs: `sdk.yml`, `playground.yml`, `server.yml`; shell & workflow lint: `actionlint.yml`; auto-update: `aztec-nightlies.yml`, `aztec-stable.yml`; infra: `infra.yml`; deploy: `deploy-mainnet.yml`, `deploy-testnet.yml`, `deploy-nightlies.yml`; reusable: `_build-base.yml`, `_deploy-unified.yml`, `_publish-sdk.yml`, `_aztec-update.yml`, `_e2e-sdk.yml`, `_e2e-playground.yml`)
