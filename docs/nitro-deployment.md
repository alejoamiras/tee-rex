...
| Resource | ID / Name |
|----------|-----------|
| ECR repo | `<ACCOUNT_ID>.dkr.ecr.eu-west-2.amazonaws.com/tee-rex` |
| Security group | `<SECURITY_GROUP_ID>` (ports 22, 4000 open) |
| IAM role | `tee-rex-ec2-role` |
| Instance profile | `tee-rex-ec2-profile` |
| Key pair | `tee-rex-key` (private key in team secrets) |
| AMI | `<AMI_ID>` (Amazon Linux 2023, x86_64) |
...
docker buildx build \
  ...
  -t <ACCOUNT_ID>.dkr.ecr.eu-west-2.amazonaws.com/tee-rex:latest \
  --push .
```
...
aws ecr get-login-password --region eu-west-2 \
  | docker login --username AWS --password-stdin \
    <ACCOUNT_ID>.dkr.ecr.eu-west-2.amazonaws.com
docker pull <ACCOUNT_ID>.dkr.ecr.eu-west-2.amazonaws.com/tee-rex:latest
docker tag <ACCOUNT_ID>.dkr.ecr.eu-west-2.amazonaws.com/tee-rex:latest tee-rex-nitro:latest
...
aws ec2 delete-security-group --group-id <SECURITY_GROUP_ID> --region eu-west-2
